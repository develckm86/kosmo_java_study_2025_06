...
public class S31AnonymousLocalVar {
    /*
class A{
    int instantI=10;//인스턴트 필드
    class 1 implements Tesable{
        public void a(){
            ++a; //불가능
            int b=a; //불가능
        }
...
다음 중 익명 클래스 내부에서 외부 지역 변수(local var)에 접근하기 위한 조건으로 옳은 것은?
B
...
A
A. 한 번만 선언되고 이후 값이 바뀌지 않아야(fianl) 한다
...
A. final 키워드가 붙은 변수 (X)
B. 처음 선언 후 값을 한 번만 대입하고 더는 변경하지 않은 변수 (O)
C. 컴파일 타임에만 접근 가능한 변수 (지역변수를 익명클래스나 람다식에서 접근)
...
A
A. 해당 지역 변수의 값을 변경해야하는 경우 (변수)
B. 해당 지역 변수가 final인 경우 (상수)
