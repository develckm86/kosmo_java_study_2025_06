...
“모든 클래스는 Object로부터 상속받는다.”
    ...
    •	자바는 다양한 타입의 객체를 같은 타입으로 다루는 기능이 필요합니다. (오늘!!)
...
🧠 핵심 개념 요약
	...
	•	가장 많이 쓰는 메서드: toString(), equals()
	•	toString()을 오버라이드하면 객체 정보를 사람이 이해하기 쉽게 만들 수 있다.(재정의용)
...
⸻
class for if import extends  "" '' {} =>자바 구현을 위한 약속된 표기 => 예약어
int class=10 x
int extends=20 x
int for=30; x => 예약어는 이름으로 사용하면 안된면
...
2. 자식 클래스에서 부모 클래스의 생성자를 호출할 때 사용하는 키워드는 무엇인가요?
2
...
자식 클래스의 생성자에서 부모클래스의 생성자를 호출한다.
⸻
...
두객체의 필드가 같은지 확인하기 위해 재정의해야합니다.
//컴퓨터 : 두 객체가 다른 주소면 다른 객체 (==)
//사람 : 두 객체의 필드가 같은 같은 객체 아닌가? 를 구현하는 메서드  equals()
⸻

10.	부모와 자식 클래스가 동일한 이름의 필드를 가질 경우, this.name과 super.name은 무엇을 의미하나요?
this.name : 자식 클래스의 필드 name을 참조
super.name : 부모 클래스의 필드 name을 참조
⸻
...
자식의 기본 생성자로는 부모의 생성자를 호출할 수 없기 때문에
자식 생성자의 맨 상단에서 부모 생성자를 호출한다.
...
public class Test {
    public static void main(String[] args) {
        Person p = new Person();
        System.out.println(p); // Person@해시코드
    }
}
...
다형성(1개인데 역할이 여러개인 것,유연한 것)의 일부로
오버로드(여러개)는 1개의 메서드 이름으로 매개변수가 다른 메서드를 생성가능
오버라이드는 부모의 메서드를 자식이 재정의 하는 것
...
class Child extends Parent {
    String name = "자식";
    void print() {
        System.out.println(this.name); "자식" /n
        System.out.println(super.name); "부모" /n
    }
}
...
클래스의 기본 설계와 유용한 메서드(toString,equals)를 제공합니다.

**타입의 다형성(유연성)으로 모든 객체를 Object 변수가 참조 가능해서 설계의 유연성을 제공합니다.  **
⸻
...
class Parent {
    String name = "부모";
    void show() {
        System.out.println("Parent: " + name); //"Parent: 부모"
    }
}
...
class Child extends Parent {
    String name = "자식";
    void show() {
        System.out.println("Child: " + name); //"Child: 자식"
    }
}
...
public class Main {
    public static void main(String[] args) {
        Child c = new Child();
        c.show();//"Child: 자식"
        System.out.println("name: " + c.name); "자식"
    }
}
...
부모의 생성자 규칙이 존재하는 자식이 부모를 호출하지 않아서

자식은 부모의 생성자를 무조건 호출해야하는데
기본생성자 호출로는 바뀐 부모의 생성자를 호출할 수 없기 때문에
명시적(작성)으로 부모생성자를 호출하도록 해야합니다.
이때 부모 생성자의 위치는 자식 생성자의 맨 위로 자식보다 먼저 생성되어야 합니다.
...
② 자식 클래스는 부모의 private 멤버를 직접 접근할 수 있다. (x)
③ 생성자에서 부모의 기본생성자 (super()) 호출은 생략 가능하다. (O)
④ super 키워드는 정적(static) 메서드 안에서도 사용할 수 있다. (x)

static 메서드 ->독립적인 존재

class A{
    int a;
    static int b; // new A() 관련이 없는 독립적인 데이터 (객체생성 없이 사용가능)

    public void sum(){ //new A() 가 될때 필드로 생성
        ++this.a; //가능
        super.toString();
    ...
    public static void main(String []args){ //A와 전형 관련이 없고 이름만 빌려씀 (객체생성 없이 사용가능)
        ++this.a; //불가능
        super.toString() //불가능
        A a=new A();
        ++a.a; //가능
...
Object.equals() : 주소가 같은지 비교
자식 클래스에서 재정의 할때는 모든 필드가 같은지 비교해야 합니다.

⸻

26. 다음 코드는 컴파일은 되지만 출력이 이상합니다. 그 이유는 무엇이며, 어떻게 개선할 수 있을까요?

toString()에서 타입과 필드를 출력하도록 재정의합니다.

@Override
public String toString(){ //문자열로 객체정보를 알려주겠어!!
...
보모의 메서드를 재정의 했다고 생각했는데 아닌 경유를 방지하게 위해(아니면 컴파일시 오류가 발생해서)
...
B의 기본 생성자가 this()로 자신을 호출해서 무한 재귀가 됩니다. => 오류발생
